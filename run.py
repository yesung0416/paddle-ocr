...
def main(ocr, video_path, output_folder_path, threshold=0.6, savefile_type='csv', draw_ocr_flag=False):
    ...
    while cap.isOpened():
        ...
        if frame_num % int(fps) == 0:
            ...
            try:
                screen_text = get_ocr_text(result_screen) if result_screen else ""
                subtitle_text = get_ocr_text(result_subtitle) if result_subtitle else ""
                screen_boxes = get_ocr_boxes(result_screen) if result_screen else []
                subtitle_boxes = get_ocr_boxes(result_subtitle) if result_subtitle else []
...
if __name__ == "__main__":
    ...
    exec_time = main(ocr, args.video_path, args.output_path, args.threshold, args.savefile_type, args.draw_ocr)
